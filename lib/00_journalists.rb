...
# Portion réservée à l'inclusion d'éventuelles bibliothèques
#


# draw_marquee - Affichage de la bannière du programme 00_journalists.rb
# IN  : -
# DO  : Affiche la bannière "JOURNALISTS v1.0"
# OUT : -
def draw_marquee
  puts  $decalage+"##################################################"
  puts  $decalage+"#                                                #"
  puts  $decalage+"#       TWITTER JOURNALISTS SPAMMERZ v1.0        #"
  puts  $decalage+"#                                                #"
  puts  $decalage+"##################################################"
end
...
# IN  : -
# DO  : Affiche de la bannière de sortie du programme
# OUT : -
...
def prompt_then_branch_user
  ...
  puts
  puts $decalage+"Saisissez votre choix, suivi de la touche [Entrée], s'il-vous-plaît ?"
  print $decalage+"  > "
  user_choice = gets.chomp

  case user_choice
  when "1"
    # Lancer count_handles
    puts $debug+"count_handles"
  when "2"
    # Lancer show_shorter_handles
    puts $debug+"show_shorter_handles"
  when "3"
    # Lancer count_handles_of_length_x
    puts $debug+"count_handles_of_length_x"
  when "4"
    # Lancer show_handlers_starting_with_cap
    puts $debug+"show_handlers_starting_with_cap"
  when "5"
    # Lancer sort_handlers_from_a_to_z
    puts $debug+"sort_handlers_from_a_to_z"
  when "6"
    # Lancer sort_handlers_by_length
    puts $debug+"sort_handlers_by_length"
  when "7"
    # Lancer find_handler
    puts $debug+"find_handler"
  when "8"
    # Lancer distrib_handlers
    puts $debug+"distrib_handlers"
  else
    $exit=TRUE
  end
  if !$exit then gets end # Permet de faire un pause (appui sur [Entrée] pour continuer)
end
